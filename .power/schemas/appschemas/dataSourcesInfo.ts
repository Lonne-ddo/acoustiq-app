...
/*
 * Généré via `pac code add-data-source`. VERSIONNÉ dans le dépôt (contrairement à
 * l'habitude des artefacts .power) car la Power Platform CLI (`pac`) est
 * indisponible sur les postes Englobe → ce fichier n'est pas régénérable et doit
 * être committé pour que `getClient(dataSourcesInfo)` route les opérations vers
 * la table `acq_acq_projets` (voir entrée en bas de fichier).
 */

...
import type { MeasurementFile, ConformiteSummary, Period as NamedPeriod, Category } from '../types'
import { computeL90 } from '../utils/acoustics'
...
// Types, référentiel (Tableau 1), PointResult et calcul de la fenêtre :
// src/utils/conformiteFenetre.ts (extraits à comportement identique).
...
export default function Conformite2026({
  ...
  const pointNames = useMemo(() => pointsActifs(files, pointMap, selectedDate), [files, pointMap, selectedDate])

  /** Données brutes regroupées par point (pré-filtrées par les périodes actives) */
  const dataByPoint = useMemo(() => donneesParPoint(files, pointMap, selectedDate, pointNames, periods, categories), [files, pointMap, selectedDate, pointNames, periods, categories])
  ...
  const results: PointResult[] = useMemo(() => evaluerFenetres({
    files, pointMap, selectedDate, periods, categories, evalHour, period, brJour, brNuit, receptor,
    ktManual, kiManual, ksEnabled, ksValue, ksReason,
  }), [
    files,
    pointMap,
    selectedDate,
